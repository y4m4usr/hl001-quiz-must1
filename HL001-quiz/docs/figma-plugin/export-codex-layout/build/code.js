...
async function buildExport(frame) {
    ...
    nodes.forEach((node, index) => {
        ...
        const designNote = getDesignNote(node);
        if (designNote) {
            layoutObject.metadata = Object.assign(Object.assign({}, layoutObject.metadata || {}), { designNote });
        }
        ...
        const isCssTemplate = isCssTemplateNode(node);
        if (isCssTemplate) {
...
function isCssTemplateNode(node) {
    ...
    const note = getDesignNote(node).toLowerCase();
    if (name.includes("css-template") || note.includes("css-template")) {
        return true;
    }
    return note.includes("css") && note.includes("html");
}
function getDesignNote(node) {
    if (node && typeof node.description === "string" && node.description.trim().length > 0) {
